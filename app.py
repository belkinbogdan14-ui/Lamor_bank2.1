...
class Product(db.Model):
    ...
    contact = db.Column(db.String(100)) # Поле контактов
    seller_id = db.Column(db.Integer)   # ID продавца для перевода
...
def get_current_data():
    ...
    if not u or u.is_frozen: return None, None
...
def home():
    ...
    notes = Notification.query.order_by(Notification.date.desc()).all()
...
def register():
    if request.method == 'POST':
        ...
        if not User.query.filter_by(login_id=l_id).first():
            u = User(login_id=l_id, name=name, password=pwd, is_admin=(User.query.count() == 0))
    ...
    if request.method == 'POST':
        ...
        if action == 'sell':
            ...
            if p and acc.balance >= p.price:
                ...
                acc.balance -= p.price
                if s_acc: s_acc.balance += p.price
                db.session.add(Notification(message=f"Товар {p.title} продан! +{p.price} ГМР от {u.name}"))
                db.session.delete(p)
        db.session.commit()
...
def transfers():
    ...
    if request.method == 'POST':
        ...
        if target_acc and acc.balance >= amount and amount > 0:
            ...
            db.session.add(Notification(message=f"Перевод: {u.name} прислал вам {amount} ГМР"))
            db.session.commit()
...
def admin():
    ...
    if request.method == 'POST':
        ...
        if act == 'post_news':
            db.session.add(Notification(message=request.form.get('news_text')))
            
        # Начислить деньги
        elif act == 'add_money':
            t_acc = Account.query.filter_by(user_id=request.form.get('user_id')).first()
            if t_acc: 
                t_acc.balance += float(request.form.get('amount', 0))
                
        # НАЗНАЧИТЬ АДМИНОМ (НОВОЕ)
        elif act == 'make_admin':
            target_user = db.session.get(User, request.form.get('user_id'))
            if target_user:
